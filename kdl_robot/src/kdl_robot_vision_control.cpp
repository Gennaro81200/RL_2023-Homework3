...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            // Improved look-at-point algorithm 
            Eigen::Matrix<double,3,1> c_P_0 = toEigen(cam_T_object.p);
            Eigen::Matrix<double,3,1> s =  c_P_0/c_P_0.norm();
            // R
            Eigen::Matrix<double,6,6> R = Eigen::Matrix<double,6,6>::Zero();
            R.block(0, 0, 3, 3) = toEigen(robot.getEEFrame().M); // = Rc
            R.block(3, 3, 3, 3) = toEigen(robot.getEEFrame().M); // = Rc
            // L
            Eigen::Matrix<double,3,6> L = Eigen::Matrix<double,3,6>::Zero();
            L.block(0, 0, 3, 3) = -(1/c_P_0.norm())*(Eigen::Matrix<double,3,3>::Identity() - s*s.transpose());
            L.block(0, 3, 3, 3) = skew(s);
            L = L*R.transpose();
            // sd
            Eigen::Matrix<double,3,1> sd;
            sd << 0, 0, 1;
            // N
            Eigen::MatrixXd LJ = L*toEigen(J_cam);
            Eigen::MatrixXd LJ_pinv = LJ.completeOrthogonalDecomposition().pseudoInverse();
            Eigen::Matrix<double,7,7> N = Eigen::Matrix<double,7,7>::Identity() - LJ_pinv*LJ;
            //dqd
            double k = 1;
            double h = 10;
            Eigen::Matrix<double,7,1> dq_0 = h*(qdi - toEigen(jnt_pos));
            dqd.data = k*LJ_pinv*sd + N*dq_0;
